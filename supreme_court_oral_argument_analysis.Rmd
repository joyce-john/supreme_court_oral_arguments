...
questions_t_test <- t.test(questions ~ voted_for_petitioner, data = all_cases_with_votes, na.action = "na.omit")
...
These two methods are different enough that they're not even really well correlated. I standardized the sentiment scores from both methods (by calculating [z-scores](https://en.wikipedia.org/wiki/Standard_score)) and plotted them against each other. Every dot represents the transcript of one justice's speech in a single case - the x and y axes show the standardized sentiment scores given by the two different systems.    

```{r afinn vs sentimentr z-score scatter, echo = FALSE, warning = FALSE, message = FALSE}

# scatter of z-scores for afinn and sentimentr
# the scores are weakly correlated within 2 standard deviations of their respective means and very different at the extremes
# geom_point_interactive must be the final layer of the plot, or else the tooltips will be covered by the geom_smooth ribbon
...
```  
Interesting. Breyer, Ginsburg, Kagan, and Sotomayor represent the liberal wing of the court. They sit at the top of this table for being the most negative, on average. The conservative justices, on the other hand, are lumped together at the bottom of this table. (If the pattern were perfect, Alito and Roberts would switch places, though.)  
